...
\item Through simulations motivated by precision agriculture scenarios, we demonstrate clear performance--communication--energy trade-offs, visualize the grow-and-reset mechanism, and benchmark against optimal scheduling oracles.
\end{itemize}
...
\begin{figure*}[t]
    ...
    The sensor (e.g., UAV) continuously measures the system state, while state updates are transmitted only when the controller-side prediction covariance exceeds a threshold~$\delta$.
    Communication imperfections such as packet loss and finite-bit quantization can be modeled.
...
To ground the analysis in a representative unmanned aerial vehicle (UAV) scenario, we consider a planar double-integrator model capturing translational motion in the $x$--$y$ plane. The state is defined as
\[
x_k \triangleq \begin{bmatrix} p_x & v_x & p_y & v_y \end{bmatrix}^\top \in \mathbb{R}^4,
\]
where $(p_x,p_y)$ and $(v_x,v_y)$ denote planar position and velocity, and the control input $u_k \in \mathbb{R}^2$ corresponds to commanded accelerations $u_k = [a_x,a_y]^\top$.

With sampling period $h>0$, the dynamics can be written as
\begin{equation}
\begin{aligned}
p_{x,k+1} &= p_{x,k} + h v_{x,k} + \tfrac{1}{2} h^2 a_{x,k}, \\
v_{x,k+1} &= v_{x,k} + h a_{x,k}, \\
p_{y,k+1} &= p_{y,k} + h v_{y,k} + \tfrac{1}{2} h^2 a_{y,k}, \\
v_{y,k+1} &= v_{y,k} + h a_{y,k},
\end{aligned}
...
This model captures the dominant translational dynamics of a UAV operating in regimes where attitude dynamics are stabilized by a fast inner-loop controller. The proposed event-triggered sensing--communication--control framework is developed for the general LTI form~\eqref{eq:lti}, while the planar model~\eqref{eq:uav_dynamics} serves as a concrete instantiation used in simulations.

\subsection{Full-State Measurement and Intermittent Updates}
...
\end{equation}
where $v_k$ denotes zero-mean measurement noise with covariance $R \succ 0$. The process disturbance $w_k$ in~\eqref{eq:lti} is assumed to be zero-mean with covariance $Q \succeq 0$. In the simulations we use full-state measurement ($C=I$) with small Gaussian noise.
...
We adopt an error-based event-triggered state update rule based on the estimation uncertainty generated by the intermittent Kalman filter. Let $P_k^{-}$ denote the prediction covariance before the measurement update. The communication indicator $\gamma_k \in \{0,1\}$ is defined as
\begin{equation}
\gamma_k =
\begin{cases}
...
\end{equation}
where $\delta > 0$ is a tunable triggering threshold. Consequently, both $P_k$ and $\gamma_k$ evolve as sequences induced by the choice of $\delta$.
...
Instead, we adopt a direct evaluation approach that exploits the low dimensionality of the design parameter. Specifically, the triggering threshold~$\delta$ is swept over a logarithmically spaced grid. For each candidate value of~$\delta$, the closed-loop system is simulated over the finite horizon~$T$ using the intermittent Kalman filter and fixed LQR controller described in Section~III. The resulting state-regulation cost $J_x(\delta)$, control energy $E_u(\delta)$, and communication usage $N_{\mathrm{tx}}(\delta)=\sum_{k=0}^{T}\gamma_k$ are recorded.
...
State estimation is performed using the intermittent Kalman filter described in Section~III. Small process and measurement disturbances are included to induce nontrivial growth of estimation uncertainty between communication events. All simulations are conducted over a finite horizon~$T$ and averaged over multiple Monte Carlo runs with randomized initial conditions. Unless otherwise stated, the communication channel is assumed lossless and unquantized.
...
Fig.~\ref{fig:time_response} shows the evolution of the prediction uncertainty $\mathrm{tr}(P_k^-)$, the state-transmission error $\|x_k-\hat{x}_k\|$, and the corresponding communication events. During intervals without communication, the estimator operates in prediction mode and both uncertainty and state-transmission error grow. Once $\mathrm{tr}(P_k^-)$ exceeds the threshold~$\delta^\star$, a communication event is triggered, the estimator is reset, and the transmission error is sharply reduced.
...
\begin{figure}[t]
    \centering
    % \includegraphics[width=0.9\linewidth]{figs/fig_B_time_response.pdf}
    \caption{Time-domain illustration of the grow-and-reset mechanism. The prediction uncertainty and state-transmission error grow between communication events and are reset once the threshold~$\delta$ is exceeded. Vertical markers indicate communication events.}
...
\begin{figure}[t]
    \centering
    % \includegraphics[width=0.85\linewidth]{figs/fig_D_oracle_comparison.pdf}
    \caption{Comparison with a finite-horizon dynamic programming (DP) oracle. The DP oracle provides an upper bound on achievable performance. The threshold-based policy closely tracks the oracle while substantially outperforming periodic transmission.}
    \label{fig:oracle}
\end{figure}
