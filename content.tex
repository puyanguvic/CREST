...
\caption{Performance--communication Pareto frontier induced by the event-triggered threshold $\delta$.
Each point corresponds to a different $\delta$, illustrating a clear trade-off between quadratic control cost and communication usage.}
\label{fig:pareto}
\end{figure}
...
\textbf{Protocol.}
We select a representative mid-range triggering threshold from Experiment~1.
For this case, we record the prediction error norm $\|\tilde{x}_k\|_2$, the system state norm $\|x_k\|_2$, and the triggering instants $\{k:\gamma_k=1\}$ during a single mission rollout.

\textbf{Results.}
Fig.~\ref{fig:time_response} visualizes the grow-and-reset mechanism.
When no update is delivered, the controller operates on model-based prediction and the prediction error grows.
Once $\|\tilde{x}_k\|_2$ exceeds $\delta$, a state update is triggered, resetting the error.
This behavior prevents unbounded error accumulation and keeps the closed-loop state bounded, in line with Theorem~\ref{thm:main}.

\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{result/fig_time_response.pdf}
\caption{Grow-and-reset mechanism in a single rollout with a representative $\delta$.
(a) The prediction error $\|\tilde{x}_k\|_2$ grows between updates and is reset when communication is triggered.
(b) The closed-loop state norm $\|x_k\|_2$ remains bounded under intermittent updates.}
...
To make the results easier to scan, the four sub-experiments and their figures are listed explicitly:
\begin{itemize}
    \item Quantization resolution sweep $\rightarrow$ Fig.~\ref{fig:quant_tradeoff}.
    \item Bit-budget sweep $\rightarrow$ Fig.~\ref{fig:budget_tradeoff}.
    \item Bursty Markov packet loss sweep $\rightarrow$ Fig.~\ref{fig:markov_robustness}.
    \item Sensitivity heatmap over $(\delta, p_{\mathrm{loss}}^{B})$ $\rightarrow$ Fig.~\ref{fig:sensitivity_heatmap}.
\end{itemize}
...
Fig.~\ref{fig:robustness_summary} summarizes the resulting budget--performance trade-off under a fixed realistic impairment setting.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{result/fig_robustness_summary.pdf}
\caption{Robustness under practical communication impairments.
Event-triggered (ET) and periodic (PER) transmission are compared in the presence of measurement noise, finite-bit quantization, and bursty Gilbert--Elliott packet loss.
ET maintains lower cost than PER across communication budgets, indicating that the advantage of state-dependent communication persists under realistic conditions.}
...
Fig.~\ref{fig:periodic_comparison} compares event-triggered and periodic baselines over multiple $\delta$ values under matched communication budgets (bits).
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{result/fig_periodic_comparison.pdf}
\caption{Event-triggered (ET) versus periodic (PER) transmission under matched communication usage.
For each triggering threshold $\delta$, the periodic period $M$ is chosen to match the average communication budget (total bits) of ET.
ET achieves lower quadratic cost than periodic transmission for the same communication budget.}
